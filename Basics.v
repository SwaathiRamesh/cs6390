...
Example t1: next_month ( January ) =  February.
...
Definition minusthree ( b : nat ) : nat :=
match b with
| O => O
| S(S O) => O
| S ( S ( S n')) => n'
...
(* if a<=b then a<b with equality removed using the successor function.
*)
...
reflexivity.
Qed.
Theorem orb_true_false : orb true false = true.
Proof.
simpl.
reflexivity.
Qed.
...
reflexivity.
Qed.
Theorem orb_false_true : orb false true = true.
Proof.
simpl.
reflexivity.
Qed.
